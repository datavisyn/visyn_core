...
export default class RDKitDrawer implements Drawer {
  ...
  constructor(canvas: HTMLCanvasElement, molecule: Molecule, config: DrawerConfig) {
    const rdkitMol = config.showScoresOnStructure ? this.getRDKitMolWithAtomNotes(molecule) : window.RDKit.get_mol(molecule.string);
    ...
    let coordinates = coordinatesFactory[coordinatesFactoryID];
    ...
    if (this.type.indexOf('black') !== -1) {
      const atomColourPalette: { [key: number]: number[] } = {};
      for (let i = 0; i < 119; i++) {
  ...
  getRDKitMolWithAtomNotes(molecule: Molecule) {
    const cxSmiles = moleculeStructureService.getCxSmilesWithScores(molecule);
    return window.RDKit.get_mol(cxSmiles);
  }

  scoresToRDKitLabels(atomScores: number[]) {
    const labels: { [id: number]: string } = {};
    atomScores.forEach((score, i: number) => (labels[i] = score.toFixed(2)));
    return labels;
  }
  ...
  setOriginalVerticesFromExternalDrawer = (
    ...
    const { smilesElements } = this.molecule;
    ...
    smilesElements.forEach((smilesElement) => {
      if (moleculeStructureService.smilesElementIsAtom(smilesElement)) {
        const coord = coordinates.find((coord) => coord.index === index)!;
        vertices.push({
          ...coord,

import { zScore, standardDeviation } from 'simple-statistics';
import { RankingType } from '../types/app.types';
import { mean, std } from '../util';
import { Method, MoleculeFromJson, ProcessedMoleculeFromJson } from '../types/molecule.types';
...
class MoleculesDatasetService {
  public getVersionControlledData(json: any): MoleculeFromJson[] {
    return json.map((molecule: any) => {
      return {
        ...
        sequence: molecule.sequence ? [...molecule.sequence] : undefined,
  ...
  private getScoresFromMolecule(molecule: MoleculeFromJson, methodName: string) {
    const method: Method | undefined = molecule.methods.find((m) => m.name === methodName);
    if (method) {
  ...
  private getScoresFromMolecules(molecules: MoleculeFromJson[], method: string) {
    return molecules
  ...
  private getSignedMeanScoreFromMoleculesNoOutliers(molecules: MoleculeFromJson[], method: string) {
    const scores = this.filterOutliers(this.getScoresFromMolecules(molecules, method));
    return {
  ...
  private getSignedStdScoreFromMoleculesNoOutliers(molecules: MoleculeFromJson[], method: string) {
    const scores = this.filterOutliers(this.getScoresFromMolecules(molecules, method));
    return {
  ...
  private getSignedMinMaxScoreFromMolecules(molecules: MoleculeFromJson[], method: string) {
  ...
  private signedZScore(
    ...
    return score > 0 ? zScore(score, sMean.positive, sStd.positive) : score < 0 ? zScore(score, sMean.negative, sStd.negative) : score;
  }

  private normalize(score: number, min: number, max: number) {
    if (max - min === 0) {
      throw new Error('Max-Min should not be 0. Please code a logic for this case. E.g., score/scores.length, or 0.5.');
    }
  ...
  private signedNormalize(
    ...
    if (score > 0) {
      ...
      return max - min === 0 ? score / vectorSize : this.normalize(score, min, max);
    }
    if (score < 0) {
  ...
  private standardizeMethodScores(molecules: MoleculeFromJson[], methodName: string) {
    // First z-score
    const sStd = this.getSignedStdScoreFromMoleculesNoOutliers(molecules, methodName);
    const sMean = this.getSignedMeanScoreFromMoleculesNoOutliers(molecules, methodName);
    const scores = this.filterOutliers(this.getScoresFromMolecules(molecules, methodName));
    const negativeScores = this.getNegativeScores(scores);
    const positiveScores = this.getPositiveScores(scores);
    const negativeMinMax = this.getMinMax(negativeScores.map((score) => this.signedZScore(score, sMean, sStd)));
    const positiveMinMax = this.getMinMax(positiveScores.map((score) => this.signedZScore(score, sMean, sStd)));
    ...
    molecules.forEach((molecule) => {
      ...
      method.scores = method.scores.map((score) => {
        if (score < 0) {
          const newScore = this.signedZScore(score, sMean, sStd) - Math.abs(negativeMinMax.max);

          const min = negativeMinMax.min - Math.abs(negativeMinMax.max);
          return newScore > 0 ? 0 : newScore < min ? min : score;
        }
        if (score > 0) {
          const newScore = this.signedZScore(score, sMean, sStd) + Math.abs(positiveMinMax.min);
          const max = positiveMinMax.max + Math.abs(positiveMinMax.min);
   ...
   */
  public preprocess = (molecules: MoleculeFromJson[], scale = false): ProcessedMoleculeFromJson[] => {
    ...
    if (copy) {
      ...
      return copy.map((moleculeFromJson: MoleculeFromJson, i: number) => {
        const scores = moleculeMethodService.scoresFromMethods(moleculeFromJson);
        const id = `${i}_${moleculeFromJson.string}_${date}`;
        return {
          ...moleculeFromJson,
          index: i,
          id,
          maxStd: Math.max(...moleculeFromJson.methods.map((method: Method) => std(method.scores))),
          std: std(scores),
          mean: mean(scores),
        };
      });
  ...
  public cloneData = (molecules: MoleculeFromJson[]): MoleculeFromJson[] => {
    return molecules ? molecules.map((mol) => moleculeStructureService.cloneMoleculeFromJson(mol)) : molecules;
  };
  ...
  public getMethodsNames(molecules: MoleculeFromJson[]) {
    const methodsNames: Set<string> = new Set();
    molecules &&
      molecules.forEach(
  ...
  sortRowsByRankingType(moleculesDataset: ProcessedMoleculeFromJson[], type: RankingType) {
    ...
    switch (type) {
      case 'ORIGINAL':
        molecules.sort((a: ProcessedMoleculeFromJson, b: ProcessedMoleculeFromJson) => a.index - b.index); // ascending
        break;
      case 'AVERAGE':
        molecules.sort((a: ProcessedMoleculeFromJson, b: ProcessedMoleculeFromJson) => b.mean - a.mean); // descending
        break;
      case 'INNER_STD':
        molecules.sort((a: ProcessedMoleculeFromJson, b: ProcessedMoleculeFromJson) => b.maxStd - a.maxStd); // descending
        break;
      case 'STD':
        molecules.sort((a: ProcessedMoleculeFromJson, b: ProcessedMoleculeFromJson) => b.std - a.std); // descending
        break;
      default: // descending
        molecules.sort((a: ProcessedMoleculeFromJson, b: ProcessedMoleculeFromJson) => {
          if (typeof a.attributes![type] === 'number') {
            const valueA = Number(a.attributes![type]);
            const valueB = Number(b.attributes![type]);
            return valueB - valueA;
          }
          const valueA = String(a.attributes![type]);
          const valueB = String(b.attributes![type]);

...
class PaletteViewService {
  public swatches = (colors: string[]) => {
    ...
    return (
      <svg viewBox={`0 0 ${n} 1`} style={{ display: 'block', width: `${n * 33}px`, height: '33px', margin: '0 -14px', cursor: 'pointer' }}>
        {colors.map((c, i) => (
          <rect key={c} x={i} width={1} height={1} fill={c} />
        ))}
      </svg>
    );
  };

  public ramp = (colors: string[], n = 300) => {

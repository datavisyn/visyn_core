...
class MoleculeMethodService {
   ...
   */
  public scoresFromMethods = (molecule: MoleculeFromJson) => molecule.methods.reduce((prev: number[], current) => [...current.scores, ...prev], []);
        ...
        molecules.forEach(
          (molecule, i) =>
            ...
            molecule.methods
              ...
              .forEach((method, j) => {
                const min = Math.min(...method.scores);
                const max = Math.max(...method.scores);
                if (domainByModel[name][0] > min) {
                  domainByModel[name][0] = min;
                }
                if (domainByModel[name][2] < max) {
                  domainByModel[name][2] = max;
                }
              }),
        );
        const minAbs = Math.abs(domainByModel[name][0]);
        const maxAbs = Math.abs(domainByModel[name][2]);
        const range = minAbs > maxAbs ? minAbs : maxAbs;
  ...
  public getDomainFromDataset(molecules: MoleculeFromJson[]) {
    const globalScoresDomain: number[] = [0, 0, 0];
    molecules &&
      molecules.forEach(
        (molecule, i) =>
          molecule != null &&
          ...
          molecule.methods.forEach((method, j) => {
            const min = Math.min(...method.scores);
            const max = Math.max(...method.scores);
            if (globalScoresDomain[0] > min) {
              globalScoresDomain[0] = min;
            }
            if (globalScoresDomain[2] < max) {
              globalScoresDomain[2] = max;
            }
          }),
      );

    const minAbs = Math.abs(globalScoresDomain[0]);
    const maxAbs = Math.abs(globalScoresDomain[2]);
    const range = minAbs > maxAbs ? minAbs : maxAbs;

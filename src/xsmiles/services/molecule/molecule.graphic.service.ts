...
class SmilesGraphicService {
  public setHoverOfAllSmilesElements(atomElements: SmilesElement[], hover = true) {
    atomElements.forEach((sElement) => (sElement.vertex!.hover = hover));
  }

  public setHoverOfBranchGivenElement(smilesElements: SmilesElement[], smilesElement: SmilesElement, hover = true) {
    const branches = smilesElement.branchesIds;
    const elementsInBranch = moleculeStructureService.getElementsIfInAllBranches(smilesElements, branches || []);
    this.setHoverOfAllSmilesElements(elementsInBranch, hover);
  }

  public setHoverOfAtom(smilesElement: SmilesElement, hover = true) {
    if (smilesElement.vertex) {
      smilesElement.vertex.hover = hover;
    }
  }

  public setHoverOfRingGivenElement(smilesElements: SmilesElement[], smilesElement: SmilesElement, hover = true) {
    const smilesElementsInRing = smilesElements.filter((e) => smilesElement.rings && smilesElement.rings.some((c) => e.rings && e.rings.includes(c)));
    this.setHoverOfAllSmilesElements(smilesElementsInRing, hover);
  }

  public setVerticesHoverStateBasedOnType(smilesElements: SmilesElement[], hoverElements: SmilesElement[], hover = true) {
    ...
    hoverElements.forEach((hoverSmilesElement) => {
      ...
      } else if (!Number.isNaN(Number(hoverSmilesElement.chars))) {

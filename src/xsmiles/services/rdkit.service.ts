...
class RDKitServices {
  ...
  private findGVertex = (gVertices: GVertex[], smilesElement: SmilesElement) => {
    return gVertices.find((v: GVertex) => v.smilesIndex === smilesElement.smilesIndex);
  };

  private createVertex = (smilesElement: SmilesElement, gVertex: any): Vertex | null => {
    if (gVertex == null) {
      return null;
    }
  ...
  public getMoleculeSVG(smiles: string, config: { width: number; height: number }) {
  ...
  private getMCommand(pathD: string): string {
    const command = this.getCommands(pathD).find((c: string) => c.includes('M'));
    return command ? command.trim() : 'M NaN,NaN';
  }

  private getLCommand(pathD: string) {
    const command = this.getCommands(pathD).find((c: string) => c.includes('L'));
  ...
  private getSource(linePath: SVGPathElement) {
    ...
    const index = parseInt(linePath.classList.item(1)!.replace('atom-', '').trim(), 10);
  ...
  private getTarget(linePath: SVGPathElement) {
    ...
    const index = parseInt(linePath.classList.item(2)!.replace('atom-', '').trim(), 10);
    return { index, x: xy[0], y: xy[1] };
  }
  ...
  private getPointsFromPath(path: SVGPathElement) {
    const [left, top, right, bottom] = getBounds(path.getAttribute('d')!);
    return [
      { x: left, y: top },
      { x: right, y: top },
      { x: left, y: bottom },
  ...
  private getVertex(index: number, pathNodes: NodeListOf<SVGPathElement>) {
    const candidatePathList = [...Array.from(pathNodes)].filter((path) => path.classList.contains(`atom-${index}`));

    const paths = candidatePathList.filter((path) => path.classList.length === 1).filter((path) => path.classList.item(0) === `atom-${index}`);
    ...
    for (let i = 0; i < candidatePaths.length; i += 2) {
      const part1 = candidatePaths[i];
      const part2 = candidatePaths[i + 1];
      if (isEven(candidatePaths.length) && this.isConnecting(part1.edge.target, part2.edge.source)) {
        const edge = {
          source: part1.edge.source,
          target: part2.edge.target,
        };
        const vertex = this.getVertexFromIndexedPath(edge, index);
        vertices.push(vertex);
      } else {
        vertices.push(this.getVertexFromIndexedPath(part1.edge, index));
        isEven(candidatePaths.length) && vertices.push(this.getVertexFromIndexedPath(part2.edge, index));
      }
    }
  ...
  private getCandidatePaths(candidatePathList: SVGPathElement[]) {
    ...
    const candidatePaths = indexedPathList.filter((p) => p.bond.trim() === selectedBond.trim());
  ...
  private getEdgeLength(path: SVGEdge) {
    const { source } = path;
    const { target } = path;
    const width = source.x > target.x ? source.x - target.x : target.x - source.x;
    const height = source.y > target.y ? source.y - target.y : target.y - source.y;
  ...
  private getVertexFromIndexedPath(candidate: SVGEdge, index: number) {
    return candidate.source.index === index ? candidate.source : candidate.target;
  }
  ...
  private getBestBond(
    ...
    for (let i = 0; i < bonds.length; i++) {
      bondSizes.push(indexedPathList.filter((p) => p.bond === bonds[i]).length);
    }
  ...
  public getMaxEdgeLength = (svg: string) => {
    ...
    return Math.max(...indexes.map((index) => this.getMaxEdgeLengthFromVertex(index, pathNodes)));
  };

  private getMaxEdgeLengthFromVertex = (index: number, pathNodes: NodeListOf<SVGPathElement>) => {
    const candidatePathList = [...Array.from(pathNodes)].filter(
  ...
  private getAtomsIndexes(pathNodes: NodeListOf<SVGPathElement>) {
    ...
    for (let i = 0; i < pathNodes.length; i++) {
      ...
      path.classList.forEach((e) => {
        if (e.includes('atom')) {
          indexSet.add(parseInt(e.replace('atom-', ''), 10));
        }
      });
    }

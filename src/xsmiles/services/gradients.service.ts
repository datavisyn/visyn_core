...
class GradientService {
  public getColorDomainWithDefaultIfEmpty(gradientConfig: GradientConfig, scores: number[]) {
  ...
  public optimizeMinMaxRadius(molecule: RawMolecule, gradientConfig: GradientConfig) {
    const { radius } = gradientConfig;

    const nAtoms: number = moleculeStructureService.getNAtomsFromSmilesString(molecule.string);
  ...
  public getGradient = (gradientConfig: GradientConfig): Gradient => {
    const { thresholds, highlight, palette, opacity, radius, delta } = gradientConfig;
    ...
    return {
      ...this.createGradient(
        thresholds,
        [0, ...thresholds, 1.0].map((threshold) => interpolator(this.adjustedThresForInterpolator(-threshold)).hex()),
        [0, ...thresholds, 1.0].map((threshold) => interpolator(this.adjustedThresForInterpolator(threshold)).hex()),
  ...
  public createGradient = (
    ...
    this.validation(thresholds, negativeColors, positiveColors, linesColors, opacity, radius, delta);
    const gradient: RawGradient = {
      colors: {
        negative: this.colorMap(negativeColors, linesColors, thresholds, delta),
        positive: this.colorMap(positiveColors, linesColors, thresholds, delta),
      },
      opacity,
      radius,
    };
  ...
  public adjustedThresForInterpolator = (signedThreshold: number) => {
    const adjusted = 0.5 + signedThreshold / 2;
    return adjusted;
  };
  ...
  colorMap(colors: string[], linesColors: string[], thresholds: number[], delta: number) {
    const map: GradientColorMap = {};
  ...
  equalColorMaps = (m1: GradientColorMap, m2: GradientColorMap): boolean => {
    return Object.keys(m1).every((key: string) => m1[parseFloat(key)] === m2[parseFloat(key)]);
  };
  ...
  validation = (
    ...
    if (!this.validColors(negativeColors, thresholds) || !this.validColors(negativeColors, thresholds)) {
      throw new Error(
  ...
  validThresholds = (thresholds: number[], delta: number) => {
    ...
    return thresholds[0] - delta > 0 && thresholds[thresholds.length - 1] + delta < 1;
  };

  validColors = (colors: string[], thresholds: number[]) => colors.length === thresholds.length + 2; // three threshold colors, plus color at 0.0, and at 1.0

  public getThresholdsList = () => {
    return [[], [0.25], [0.5], [0.75], [0.25, 0.5], [0.5, 0.75], [0.25, 0.75], [0.25, 0.5, 0.75]];
  };

  public getGradientConfig = (config?: GradientConfigOverwriteDefaults, bondLength?: number): GradientConfig => {
    if (config == null) {

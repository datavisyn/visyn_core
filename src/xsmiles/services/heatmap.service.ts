...
class HeatmapService {
  public appendHeatmap = (parent: HTMLElement, molecule: Molecule, gradient: Gradient, scaleResolution: number) => {
    ...
    const minRadius = gradient.radius.min / scaleResolution;
    const maxRadius = gradient.radius.max / scaleResolution;
    ...
    const elementsPos = vertices
      ...
      .map((vertex: Vertex) => {
        return {
          x: Math.round(vertex.position.x / scaleResolution),
          y: Math.round(vertex.position.y / scaleResolution),
          value: this.adaptValueToBeOverThresholds(scalePos(vertex.atomSmilesElement.score), gradient),
    ...
    const elementsNeg = vertices
      ...
      .map((vertex) => {
        return {
          x: Math.round(vertex.position.x / scaleResolution),
          y: Math.round(vertex.position.y / scaleResolution),
          value: this.adaptValueToBeOverThresholds(scaleNeg(vertex.atomSmilesElement.score), gradient),
  ...
  private adaptValueToBeOverThresholds(value: number, gradient: Gradient) {

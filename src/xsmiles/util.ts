export const std = (vector: number[]) => {
  ...
  const squaredDiffs = vector.map((x) => (x - mean) ** 2);
...
export const equalNumericLists = (list1: number[], list2: number[], precision = 0.00001) => {
  return !list1.some((value1) => list2.some((value2) => Math.abs(value1 - value2) > precision));
};
...
export const isEmptyNullUndefined = <T>(array: T[] | undefined | null): boolean => array == null || array.length === 0;
...
export const absoluteMaxFromVector = (numbers: number[]) => Math.max(...numbers.map((v) => Math.abs(v)));

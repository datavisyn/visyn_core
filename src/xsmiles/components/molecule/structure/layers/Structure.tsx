...
class Structure extends React.PureComponent<Props, State> {
  ...
  componentDidMount() {
    const auxMolecule = moleculeStructureService.cloneMolecule(this.props.molecule);
    const drawer = drawerService.createDrawer(this.ref.current, this.props.molecule, this.props.drawerConfig) as Drawer;
  ...
  componentDidUpdate() {
    ...
    if (
      drawer !== null &&
      (this.props.drawerConfig.drawerType !== drawer.type ||
        (this.props.drawerConfig.drawerType.includes('RDKit') &&
          this.props.drawerConfig.showScoresOnStructure !== (drawer as RDKitDrawer).showScoresOnStructure))
    ) {
      const canvas = this.ref.current;
      this.resetCanvas(canvas);
      drawer = drawerService.createDrawer(this.ref.current, molecule, this.props.drawerConfig) as Drawer;
  ...
  render() {
    return (
      <canvas
        ...
        style={{ zIndex: 4, position: 'relative', width: '100%', height: '100%' }}
      />
    );
  }
}
...
function areEqual(prevProps: Props, nextProps: Props) {
  if (prevProps.molecule !== nextProps.molecule) {
    return false;
  }
  ...
  if (prevProps.drawerConfig.structureColor !== nextProps.drawerConfig.structureColor) {
    return false;
  }
  if (prevProps.drawerConfig.width !== nextProps.drawerConfig.width) {
    return false;
  }
  if (prevProps.drawerConfig.showScoresOnStructure !== nextProps.drawerConfig.showScoresOnStructure) {
    return false;
  }

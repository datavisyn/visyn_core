...
import { scaleLinear, max, min, color as d3Color } from 'd3v7';
...
class SmilesChar extends React.Component<Props, SmilesCharState> {
  ...
  updateAlphaScaler(props: Props) {
    const positiveDomain = props.colorsDomain.map((v) => Math.abs(v)).sort();
    const alphaDomain = [0, max(positiveDomain)!];
    const scaler = scaleLinear().domain(alphaDomain).range(props.alphaRange);
    this.alphaScale = (alpha: number) => scaler(Math.abs(alpha));
  }

  updateColorScale(props: Props) {
    this.colorScale = (value: number) => colorsService.interpolate(props.colorsRange, props.colorsDomain)(value).hex();
  }

  updateBarScale(props: Props) {
    const positiveDomain = props.colorsDomain.map((v) => Math.abs(v)).sort();
    const alphaDomain = [0, max(positiveDomain)!];
    this.barScale = scaleLinear<number>().domain(alphaDomain).range([0.25, this.maxBarSize]);
  }
  ...
  render() {
    ...
    const { width, height, score, char, x, fontSize } = this.props;

    const charY = this.maxBarSize + fontSize / 2;
    const barY1 = this.maxBarSize;

    const { colorsDomain } = this.props;
    ...
    const opacity = this.state.hover || this.props.hover ? 1.0 : this.alphaScale(score);

    const color = cScale; // limit the color mapping to the max range of colors - it was reaching colors outside the colorRange.

    // todo -bar size scale score...

    const maxDomainAbs = Math.max(Math.abs(maxDomain!), Math.abs(minDomain!));
    const absScaledScore = Math.abs(score) / maxDomainAbs;
    ...
    deltaStrokeScore = deltaStrokeScore <= maxDomainAbs / 2 ? maxDomainAbs / 2 : deltaStrokeScore >= maxDomainAbs ? maxDomainAbs : deltaStrokeScore;
    strokeColor =
      score < 0
      ...
      <g style={style} transform={`translate(${x},${0})`}>
        <g>
          <text
            ...
            x={width / 2}
            y={charY}
            fill={d3Color(strokeColor)?.darker(2.5).formatHex()}
            fontSize={fontSize}
          ...
          <g transform={`translate(${width / 2},${charY + 10}) rotate(90)`}>
            <text
              opacity={opacity}
              fill={d3Color(strokeColor)?.darker(1.5).formatHex()}
              fontSize={fontSize / 1.7}
              dominantBaseline="central"
              fontFamily="'Courier New', monospace"
              textAnchor="start"
            >
              {score === 0 ? `\u00A00` : score > 0 ? `\u00A0${score.toFixed(2)}` : score.toFixed(2)}
            </text>
          </g>

          {(this.state.hover || this.props.hover) && (
            <line x1={fontSize / 2} y1={barY1} x2={fontSize / 2} y2={barY1 - this.barScale(Math.abs(score))} stroke="white" strokeWidth={width / 1.5} />
          )}
...
function areEqual(prevProps: Props, nextProps: Props) {
  if (prevProps.hover !== nextProps.hover) {
    return false;
  }
  ...
  if (prevProps.fontSize !== nextProps.fontSize) {
    return false;
  }
  if (prevProps.colorsDomain !== nextProps.colorsDomain) {
    return false;
  }
  if (prevProps.colorsRange[0] !== nextProps.colorsRange[0] || prevProps.colorsRange[2] !== nextProps.colorsRange[2]) {
    return false;
  }
  if (prevProps.alphaRange !== nextProps.alphaRange) {
    return false;
  }
  return true;
}

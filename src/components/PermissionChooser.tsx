  ...
  return (
    <div className="btn-group col-sm-auto" role="group">
      ...
      <label htmlFor={`btnradio_permissions_${id}_none`} className="form-label btn btn-outline-secondary btn-sm">
        <i className="fas fa-ban" /> {I18nextManager.getInstance().i18n.t('visyn:permission.noPermission')}
      </label>
      <input
        type="radio"
        className="btn-check"
      ...
      <label htmlFor={`btnradio_permissions_${id}_read`} className="form-label btn btn-outline-secondary btn-sm">
        <i className="fas fa-eye" /> {I18nextManager.getInstance().i18n.t('visyn:permission.read')}
      </label>
      <input
        type="radio"
        className="btn-check"
      ...
      <label htmlFor={`btnradio_permissions_${id}_write`} className="form-label btn btn-outline-secondary btn-sm">
        <i className="fas fa-edit" /> {I18nextManager.getInstance().i18n.t('visyn:permission.write')}
      </label>
    </div>
  );
}
    ...
    <>
      <div className="form-check form-check-inline">
        ...
        <label className="form-label form-check-label" htmlFor={`global_permission_private_${id}`}>
          {' '}
          <i className="fas fa-user" /> {I18nextManager.getInstance().i18n.t('visyn:permission.private')}
        </label>
      </div>
      <div className="form-check form-check-inline">
        <input
        ...
        <label className="form-label form-check-label" htmlFor={`global_permission_public_${id}`}>
          <i className="fas fa-users" /> {I18nextManager.getInstance().i18n.t('visyn:permission.publicMsg')}
        </label>
      </div>
      ...
      <button type="button" name="permission_advanced" className="btn btn-outline-secondary btn-sm float-end" onClick={() => setAdvancedOpen((o) => !o)}>
        {I18nextManager.getInstance().i18n.t('visyn:permission.advanced')}
      </button>
      {extra}
      <div className={advancedOpen ? 'd-block' : 'd-none'}>
        <div className="tdp-permissions-entry row d-flex align-items-center mt-1">
          ...
          <label className="form-label col-sm-auto ps-2">{I18nextManager.getInstance().i18n.t('visyn:permission.public')}</label>
          <span />
          <PermissionsEntry permission={permission} setPermission={setPermission} setGetter={(p) => p.others} />
        </div>

        <p className="form-text">{I18nextManager.getInstance().i18n.t('visyn:permission.definePermissions')}</p>

        <div className="tdp-permissions-entry row d-flex align-items-center mt-1">
          <label className="form-label col-sm-auto ps-2" htmlFor={`permission_group_name_${id}`}>
            {I18nextManager.getInstance().i18n.t('visyn:permission.group')}
          </label>
          <select
        ...
        <p className="form-text">{I18nextManager.getInstance().i18n.t('visyn:permission.specifyRole')}</p>

        <div className="tdp-permissions-entry row d-flex align-items-center mt-1">
          <label className="form-label col-sm-auto ps-2" htmlFor={`permission_buddies_name_${id}`}>
            {I18nextManager.getInstance().i18n.t('visyn:permission.buddies')}
          </label>
          <input
            id={`permission_buddies_name_${id}`}
            name="permission_buddies_name"
            className="form-control form-control-sm"
            placeholder={I18nextManager.getInstance().i18n.t('visyn:permission.buddiesPlaceholder')}
            value={buddies.join(';')}
        ...
        <p className="form-text">{I18nextManager.getInstance().i18n.t('visyn:permission.buddiesDescription')}</p>
      </div>
    </>
  );
}

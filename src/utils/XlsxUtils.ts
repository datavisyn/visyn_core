...
export class XlsxUtils {
  static xlsx2json(file: File): Promise<IXLSXJSONFile> {
    ...
    return appContext.sendAPI('/api/xlsx/to_json', data, 'POST');
  }
  ...
  static xlsx2jsonArray(file: File): Promise<any[][]> {
    ...
    return appContext.sendAPI('/api/xlsx/to_json_array', data, 'POST');
  }

  static json2xlsx(file: IXLSXJSONFile): Promise<Blob> {
    return Ajax.send(appContext.api2absURL('/api/xlsx/from_json'), file, 'POST', 'blob', 'application/json');
  }

  static jsonArray2xlsx(file: any[][]): Promise<Blob> {
    return Ajax.send(appContext.api2absURL('/api/xlsx/from_json_array'), file, 'POST', 'blob', 'application/json');
  }
}

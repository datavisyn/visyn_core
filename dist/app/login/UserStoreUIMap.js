...
export function DefaultLoginForm({ setError, store }) {
    return (React.createElement(VisynLoginForm, { onLogin: async (username, password) => {
            setError(null);
...
export function AutoLoginForm({ setError, store }) {
    const [loginInProgress, setLoginInProgress] = React.useState(false);
    const POLL_INTERVAL = 500;
    const MAX_POLL_TIME = 5 * 60 * 1000; // 5 minutes in milliseconds
    const HOSTNAME = window.location.hostname;
    const pollQueryStringChange = async (popup) => {
    ...
    const login = async () => {
        if (!loginInProgress) {
            ...
            try {
                ...
                popup = window.open(authUrl, 'Authorization', 'toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=no,resizable=no,width=600,height=600');
                if (!popup) {
    ...
    return (React.createElement(Text, { align: "justify" },

...
from .security import current_user, is_logged_in
...
def inject_where_clause(builder, clause):
    ...
    if index >= 0:
        ...
        builder.query(f"{query[:index]} ({clause}) AND {query[index:]}")
    else:
        before = -1
        ...
        if before < 0:
            ...
            builder.query(f"{query[:index]} WHERE {clause} {query[index:]}")
    return builder
...
def inject_where(builder):
    ...
    if where >= 0:
        if before < 0:
            ...
            builder.query(f"{query[:before]} {{and_where}} {query[before:]}")
        builder.replace("and_where")
        query = builder.v.query
        builder.query(f"{query[:where]} {{joins}} {query[where:]}")
        builder.replace("joins")
    else:
        if before < 0:
            builder.append("{joins} {where}")
        else:
            builder.query(f"{query[:before]} {{joins}} {{where}} {query[before:]}")
...
def add_common_queries(
    ...
    queries[prefix + "_items_verify"] = (
        ...
        .call(call_function)
        .call(inject_where)
        .filter(name_column, f"lower({name_column}) {{operator}} {{value}}")
        .build()
    )
